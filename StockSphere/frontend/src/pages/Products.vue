  ...
  <div class="products-container">
          ...
          <td>
            ...
            <ul>
              <li v-for="(usage, index) in product.resource_usages" :key="index">
                {{ getResourceName(usage.resourceId) }}
              </li>
            </ul>
          </td>
    ...
    <div v-if="showProductDetailsModal" class="modal-overlay">
      <div class="modal">
        ...
        <button class="close-button" @click="showProductDetailsModal = false">Close</button>
      </div>
    </div>
  </div>
</template>
...
<script setup>
  ...
  import { PencilLine, Trash2, Info } from "lucide-vue-next";
  ...
  // Define the fmtGBP function to format GBP properly
